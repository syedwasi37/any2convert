...
function getTournamentBracketGeneratorHTML() {
    return <<<'HTML'
    <style>
        .bq{max-width:92rem;margin:0 auto;display:grid;grid-template-columns:minmax(320px,360px) minmax(0,1fr);gap:1.5rem}
        .bq-side,.bq-board{border:1px solid rgba(148,163,184,.12);box-shadow:0 24px 80px rgba(15,23,42,.28)}
        .bq-side{border-radius:2rem;padding:1.5rem;background:linear-gradient(180deg,rgba(17,24,39,.96),rgba(15,23,42,.98));color:#f8fafc}
        .bq-card{border-radius:1.4rem;padding:1rem;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06)}
        .bq-field{width:100%;margin-top:.7rem;border-radius:1rem;border:1px solid rgba(255,255,255,.08);background:rgba(15,23,42,.8);color:#f8fafc;padding:.9rem 1rem;font-weight:700;outline:none}
        .bq-board{border-radius:2rem;padding:1.5rem;min-height:48rem;color:#fff;background:radial-gradient(circle at top left,rgba(59,130,246,.1),transparent 22rem),linear-gradient(180deg,#1d2128 0%,#141821 100%)}
        .bq-canvas{overflow:auto;margin-top:1.2rem;padding:1rem;border-radius:1.5rem;background:linear-gradient(transparent 31px,rgba(255,255,255,.03) 32px),linear-gradient(90deg,transparent 31px,rgba(255,255,255,.03) 32px);background-size:32px 32px}
        .bq-rounds{display:flex;gap:3rem;align-items:flex-start;min-width:max-content}
        .bq-round{min-width:17rem;display:flex;flex-direction:column}
        .bq-match{position:relative;width:15rem;height:5.5rem}.bq-box{height:100%;display:grid;grid-template-rows:1fr 1fr;background:rgba(18,22,31,.98);border:1px solid rgba(255,255,255,.1);border-radius:1.05rem;overflow:hidden;position:relative;z-index:2}
        .bq-row{display:grid;grid-template-columns:auto 1fr auto;gap:.6rem;align-items:center;padding:.65rem .8rem}.bq-row+.bq-row{border-top:1px solid rgba(255,255,255,.08)}
        ...
        .bq-seed,.bq-score{font-size:.84rem;font-weight:900}.bq-name{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:.95rem;font-weight:800}
        .bq-pill{position:absolute;top:-.8rem;left:.7rem;z-index:3;padding:.24rem .55rem;border-radius:999px;font-size:.64rem;letter-spacing:.12em;text-transform:uppercase;background:rgba(14,165,233,.16);border:1px solid rgba(56,189,248,.18);color:#7dd3fc;font-weight:800}
        .bq-h,.bq-v{position:absolute;background:rgba(255,255,255,.72);z-index:1}.bq-h{top:calc(50% - 1px);left:calc(100% + .15rem);width:1.9rem;height:2px}.bq-v{left:calc(100% + 2rem);width:2px;height:var(--span)}.bq-v.down{top:50%}.bq-v.up{top:calc(50% - var(--span))}
        .bq-stat{border-radius:1.1rem;padding:.95rem;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06)}
        ...
        .bq-sched{border-radius:1rem;padding:.9rem 1rem;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06)}
        @media (max-width:1279px){.bq{grid-template-columns:1fr}}
    </style>
    <div class="bq">
        <aside class="bq-side">
...
Team FNS</textarea>
        ...
        <section class="bq-board">
            <div class="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-white/8"><div><p class="text-[11px] tracking-[0.28em] uppercase text-cyan-300 font-semibold">Single Elimination</p><h3 id="bracketBoardTitle" class="mt-2 text-3xl font-black">Valorant Tourney</h3><p id="bracketBoardMeta" class="mt-2 text-sm text-slate-300">8 teams, randomized seeds, opening matches scheduled automatically.</p></div><div class="flex flex-wrap gap-3"><div class="bq-stat min-w-[8rem]"><p class="text-[11px] uppercase tracking-[0.22em] text-slate-400">Bracket Size</p><p id="bracketBoardSize" class="mt-2 text-2xl font-black">8</p></div><div class="bq-stat min-w-[8rem]"><p class="text-[11px] uppercase tracking-[0.22em] text-slate-400">Matches</p><p id="bracketBoardMatches" class="mt-2 text-2xl font-black">7</p></div></div></div>
            <div class="bq-canvas"><div id="bracketCanvas" class="bq-rounds"></div></div>
            <div class="mt-5 rounded-[1.4rem] border border-white/8 bg-white/5 p-4"><div class="flex items-center justify-between gap-4"><div><p class="text-[11px] uppercase tracking-[0.22em] text-cyan-300">Schedule Feed</p><h4 class="mt-1 text-xl font-black text-white">Randomized Match Schedule</h4></div><p id="bracketScheduleMeta" class="text-sm text-slate-400">Generated from team list and match slot length</p></div><div id="bracketSchedule" class="mt-4 grid lg:grid-cols-2 gap-3"></div></div>
        </section>
    </div>
    <script>
        (() => {
        ...
        imgInput.addEventListener("change", function() {
            ...
            Array.from(this.files).forEach(file => {
                ...
                reader.onload = function(e) {
                    ...
                    div.innerHTML = `<img src="${e.target.result}" class="w-full h-24 object-cover rounded-lg" alt="convert image to pdf"><span class="absolute top-0 right-0 bg-green-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">âœ“</span>`;
